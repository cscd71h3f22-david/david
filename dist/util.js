...
(function (utils) {
    ...
    utils.cronStringFromDate = (date) => `${date.getSeconds()} ${date.getMinutes()} ${date.getHours()} ${date.getDate()} ${date.getMonth() + 1} ${date.getDay()}`;
    /**
     ...
     * @returns Timeout
     */
    utils.setTimeout = (fn, ms, ...args) => {
        const timeout = timeoutCount++;
        let timer;
        if (ms > MAX_TIMEOUT_MS) {
            // Use cron task
            const triggerDate = new Date(Date.now() + ms);
            const cronTask = node_cron_1.default.schedule(utils.cronStringFromDate(triggerDate), () => {
                fn(args);
            });
